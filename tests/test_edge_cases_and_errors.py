...
class TestAuthEdgeCases:
    ...
    def test_get_api_key_case_sensitive(self, app_client):
        ...
        wrong_case = plaintext.swapcase() if plaintext != plaintext.swapcase() else "different"
    ...
    def test_log_event_with_long_details(self, app_client):
        ...
        rows = db.fetch_all("SELECT * FROM api_key_audit WHERE api_key_id = ?", (key["id"],))
        assert len(rows) > 0
...
class TestUserManagementEdgeCases:
    ...
    def test_multiple_concurrent_login_failures(self, app_client):
        ...
        user_row = db.fetch_one("SELECT failed_login_attempts FROM users WHERE email = ?", (email,))
    ...
    def test_token_with_missing_expiration(self, app_client):
        ...
        no_exp_token = jwt.encode(no_exp_payload, user_mgmt.JWT_SECRET, algorithm=user_mgmt.JWT_ALG)
...
class TestDatabaseEdgeCases:
    ...
    def test_fetch_all_empty_table(self, app_client):
        result = db.fetch_all("SELECT * FROM api_key_audit WHERE api_key_id = ?", (999999,))
        assert result == []
...
class TestPasswordResetEdgeCases:
    ...
    def test_reset_password_wrong_token_type(self, app_client):
        ...
        wrong_type_token = jwt.encode(wrong_type_payload, user_mgmt.JWT_SECRET, algorithm=user_mgmt.JWT_ALG)
