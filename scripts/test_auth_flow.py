...
conn.request('POST', '/api/auth/login', body=payload, headers={'Content-Type': 'application/json'})
...
access = login_data.get('tokens', {}).get('access_token') or login_data.get('access_token')
if not access:
